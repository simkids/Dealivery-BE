...
class ProductBoardServiceTest {
	...
	@DisplayName("[해피 케이스] 파라미터 값으로 게시글을 생성한다.")
	@Test
	void createBoard() {
		// given
		...
		ProductBoard savedProductBoard =
			productBoardService.create(
				1L,
				productBoardRequest,
				new MultipartFile[]{},
				new MockMultipartFile("detailURL", new byte[]{})
			);
	...
	@DisplayName("[예외 케이스] 게시글 생성 시 대기열 생성을 실패하면 예외가 발생한다")
	@Test
	void createBoardWithNullQueue() {
		// given
		List<ProductDto.Request> productDtoRequests = List.of(createProductRequest(1000), createProductRequest(2000));
		ProductBoardDto.BoardCreateRequest productBoardRequest = createBoardRequest(productDtoRequests, CategoryType.CLOTHES);
		...
		// when & then
		assertThatThrownBy(() -> productBoardService.create(
			1L,
			productBoardRequest,
			new MultipartFile[]{},
			new MockMultipartFile("detailURL", new byte[]{})
			...
			.hasMessage(BaseResponseStatus.PRODUCT_BOARD_QUEUE_CREATE_FAIL.getMessage());
	}
